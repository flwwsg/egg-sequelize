...
module.exports = app => {
  app.addSingleton('sequelize', createClient);
};
...
async function createClient(config, app) {
  const defaultConfig = {
    ...
    logging(...args) {
      ...
      let bind = args[2] && args[2].bind || '';
      if (bind) {
        bind = bind.join(',');
      }
      app.logger.info('[sequelize]%s %s', used, args[0], bind);
    },
    host: 'localhost',
    port: 3306,
    username: 'root',
  ...
  const client = loadDatabase(Object.assign({}, defaultConfig, config));
  // const databases = [];
